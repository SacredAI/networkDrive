...
class AppleHelpBuilder(StandaloneHTMLBuilder):
    ...
    def build_info_plist(self, contents_dir: str) -> None:
        ...
        if self.config.applehelp_icon is not None:
            info_plist['HPDBookIconPath'] = path.basename(
                self.config.applehelp_icon)
        ...
        if self.config.applehelp_icon:
            ...
            try:
                with progress_message(__('copying icon... ')):
                    applehelp_icon = path.join(self.srcdir,
                                               self.config.applehelp_icon)
                    copy_asset_file(applehelp_icon, resources_dir)
            except Exception as err:
                logger.warning(__('cannot copy icon file %r: %s'), applehelp_icon,
                               err)
    ...
    def build_access_page(self, language_dir: str) -> None:
        ...
        copy_asset_file(path.join(template_dir, '_access.html_t'), language_dir,
                        context)
    ...
    def do_codesign(self) -> None:
        ...
        if self.config.applehelp_disable_external_tools:
            ...
            try:
                ...
                raise AppleHelpCodeSigningFailed(
                    __('Command not found: %s') % args[0])
            except CalledProcessError as exc:
...
def setup(app: Sphinx) -> Dict[str, Any]:
    ...
    app.add_config_value('applehelp_locale', lambda self: self.language or 'en',
                         'applehelp')
    app.add_config_value('applehelp_title', lambda self: self.project + ' Help',
                         'applehelp')
