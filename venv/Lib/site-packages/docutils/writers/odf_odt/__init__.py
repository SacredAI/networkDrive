...
class ODFTranslator(nodes.GenericNodeVisitor):
    ...
    def retrieve_styles(self, extension):
        ...
        if ext == '.xml':
            ...
            raise RuntimeError('stylesheet path (%s) must be %s or .xml file' % (
                stylespath, extension))
    ...
    def visit_classifier(self, node):
        ...
        if len(els) > 0:
            ...
            el1 = SubElement(el, 'text:span',
                             attrib={
                                 'text:style-name': self.rststyle('emphasis')
                             })
    ...
    def visit_literal_block(self, node):
        ...
        if lines[-1] == '':
            del lines[-1]
        lines1 = [
            '<wrappertag1 xmlns:text="urn:oasis:names:tc:opendocument:xmlns:text'
            ':1.0">']
        ...
        if 'format' in node.attributes:
            ...
            if 'odt' in formatlist:
                ...
                attrstr = ' '.join(['%s="%s"' % (k, v,)
                                    for k, v in
                                    list(CONTENT_NAMESPACE_ATTRIB.items())])
        ...
        if table_name is not None:
            ...
            if table_style is None:
                ...
                if table_style is None:
                    ...
                    self.document.reporter.warning(
                        'Can\'t find default table style "%s".  Using built-in '
                        'default.' % (
                            table_name,))
        ...
        if 'classes' in node.attributes:
            if 'contents' in node.attributes['classes']:
                ...
                if self.settings.generate_oowriter_toc:
                    ...
                    el1 = SubElement(el, 'text:span',
                                     attrib={
                                         'text:style-name': self.rststyle('strong')
                                     })
                    el1.text = label
