    ...
    def translate(self):
        ...
        try:
            ...
            template_file = open(os.path.join(self.default_template_path,
                                              self.document.settings.template),
                                 'rb')
...
class Babel(object):
    ...
    language_codes = dict(
        [(k.lower(), v) for (k, v) in list(language_codes.items())])
...
\ifx\DUclass\undefined % poor man's "provideenvironment"
 \newenvironment{DUclass}[1]%
  {\def\DocutilsClassFunctionName{DUCLASS#1}% arg cannot be used in end-part of 
  environment.
    ...
    def encode(self, text):
        ...
        * Separate ``-`` (and more in literal text) to prevent input ligatures.
        ...
        if self.insert_non_breaking_blanks:
            table[ord(' ')] = r'~'
        # Unicode replacements for 8-bit tex engines (not required with
        # XeTeX/LuaTeX):
        if not self.is_xetex:
    ...
    def visit_entry(self, node):
        ...
        if 'morerows' in node and 'morecols' in node:
            raise NotImplementedError('Cells that '
                                      'span multiple rows *and* columns currently '
                                      'not supported, sorry.')
            # TODO: should be possible with LaTeX, see e.g.
            # http://texblog.org/2012/12/21/multi-column-and-multi-row-cells-in
            # -latex-tables/
