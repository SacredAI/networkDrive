...
try:
    ...
    try:
        ...
        # (cf. Update 1 in http://stackoverflow.com/questions/4082645/using
        # -python-2-xs-locale-module-to-format-numbers-and-currency)
        ...
        try:
            ...
            if isinstance(self.data, EnvironmentError):
                return "[Errno %s] %s: '%s'" % (self.data.errno,
                                                SafeString(self.data.strerror,
                                                           self.encoding,
                                                           self.decoding_errors),
                                                SafeString(self.data.filename,
                                                           self.encoding,
                                                           self.decoding_errors))
            if isinstance(self.data, Exception):
                args = [str(SafeString(arg, self.encoding,
