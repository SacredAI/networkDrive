...
def get_unit_name(measurement_unit, length='long', locale=LC_NUMERIC):
    ...
    :param measurement_unit: the code of a measurement unit.
                             Known units can be found in the CLDR Unit Validity
                             XML file:
                             http://unicode.org/repos/cldr/tags/latest/common
                             /validity/unit.xml
...
def format_unit(value, measurement_unit, length='long', format=None,
    ...
    :param value: the value to format. If this is a string, no number formatting
    will be attempted.
    :param measurement_unit: the code of a measurement unit.
                             Known units can be found in the CLDR Unit Validity
                             XML file:
...
def _find_compound_unit(numerator_unit, denominator_unit, locale=LC_NUMERIC):
    ...
    # If either was not found, we can't possibly build a suitable compound unit
    # either.
    if not (numerator_unit and denominator_unit):
        return None

    # Since compound units are named "speed-kilometer-per-hour", we'll have to
    # slice off
...
def format_compound_unit(
    ...
    Both unit specifiers are optional to allow for formatting of arbitrary values
    still according
    ...
    >>> format_compound_unit(format_currency(35, "JPY", locale="de"),
    denominator_unit="liter", locale="de")
    ...
    :param numerator_value: The numerator value. This may be a string,
                            in which case it is considered preformatted and the
                            unit is ignored.
    ...
    if numerator_unit and denominator_unit and denominator_value == 1:
        compound_unit = _find_compound_unit(numerator_unit, denominator_unit,
                                            locale=locale)
        if compound_unit:
            return format_unit(numerator_value, compound_unit, length=length,
    ...
    if isinstance(numerator_value, string_types):  # Numerator is preformatted
        ...
        formatted_numerator = format_unit(
            numerator_value, numerator_unit, length=length, format=format,
            locale=locale
        )
    else:  # Unitless numerator
        formatted_numerator = format_decimal(numerator_value, format=format,
                                             locale=locale)
    ...
    if isinstance(denominator_value, string_types):  # Denominator is preformatted
        ...
        if denominator_value == 1:  # support perUnitPatterns when the
            # denominator is 1
            denominator_unit = _find_unit_pattern(denominator_unit, locale=locale)
            per_pattern = locale._data["unit_patterns"].get(denominator_unit,
                                                            {}).get(length, {}).get(
                "per")
            if per_pattern:
                return per_pattern.format(formatted_numerator)
            # See TR-35's per-unit pattern algorithm, point 3.2.
            # For denominator 1, we replace the value to be formatted with the
            # empty string;
            # this will make `format_unit` return " second" instead of "1 second".
            denominator_value = ""

        formatted_denominator = format_unit(
            denominator_value, denominator_unit, length=length, format=format,
            locale=locale
        ).strip()
    else:  # Bare denominator
        formatted_denominator = format_decimal(denominator_value, format=format,
                                               locale=locale)

    per_pattern = locale._data["compound_unit_patterns"].get("per", {}).get(length,
                                                                            "{0}/{1}")
