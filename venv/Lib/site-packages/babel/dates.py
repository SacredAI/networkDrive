...
def _ensure_datetime_tzinfo(datetime, tzinfo=None):
    ...
    >>> _ensure_datetime_tzinfo(datetime(2015, 1, 1, 13, 15, tzinfo=UTC),
    tzinfo=tz).hour
    14
...
def get_day_names(width='wide', context='format', locale=LC_TIME):
    ...
    :param width: the width to use, one of "wide", "abbreviated", "short" or
    "narrow"
    :param context: the context, either "format" or "stand-alone"
    :param locale: the `Locale` object, or a locale string
    """
...
def get_timezone_location(dt_or_tzinfo=None, locale=LC_TIME, return_city=False):
    ...
    if not return_city and territory and len(
            get_global('territory_zones').get(territory, [])) == 1:
...
def format_skeleton(skeleton, datetime=None, tzinfo=None, fuzzy=True,
                    locale=LC_TIME):
    ...
    u'Sun, Apr 1'
    >>> format_skeleton('yMMd', t, locale='fi')  # yMMd is not in the Finnish
    locale; yMd gets used
    u'1.4.2007'
    >>> format_skeleton('yMMd', t, fuzzy=False, locale='fi')  # yMMd is not in
    the Finnish locale, an error is thrown
...
def _format_fallback_interval(start, end, skeleton, tzinfo, locale):
    ...
    elif all((isinstance(d, date) and not isinstance(d, datetime)) for d in
             (start, end)):  # Both are just dates
        format = lambda dt: format_date(dt, locale=locale)
    elif all((isinstance(d, time) and not isinstance(d, date)) for d in
             (start, end)):  # Both are times
...
def format_interval(start, end, skeleton=None, tzinfo=None, fuzzy=True,
                    locale=LC_TIME):
    """
    ...
    if skeleton not in interval_formats or not skeleton:
        ...
        if not skeleton:  # Still no match whatsoever?
            # > Otherwise, format the start and end datetime using the fallback
            # pattern.
...
def get_period_id(time, tzinfo=None, type=None, locale=LC_TIME):
    ...
    :param type: The period type to use. Either "selection" or None.
                 ...
                 “Your email arrived yesterday evening” or “Your email arrived
                 last night”.
    :param locale: the `Locale` object, or a locale string
    :return: period ID. Something is always returned -- even if it's just "am" or
    "pm".
    """
    time = _get_time(time, tzinfo)
...
class DateTimeFormat(object):
    ...
    def format_weekday(self, char='E', num=4):
        ...
        'E': Day of week - Use one through three letters for the abbreviated day
        name, four for the full (wide) name,
             five for the narrow name, or six for the short name.
        >>> format.format_weekday('E',2)
        u'Sun'

        'e': Local day of week. Same as E except adds a numeric value that will
        depend on the local starting day of the
             week, using one or two letters. For this example, Monday is the
             first day of the week.
        >>> format.format_weekday('e',2)
        '01'

        'c': Stand-Alone local day of week - Use one letter for the local numeric
        value (same as 'e'), three for the
             abbreviated day name, four for the full (wide) name, five for the
             narrow name, or six for the short name.
    ...
    def format_period(self, char):
        ...
        for width in ('wide', 'narrow', 'abbreviated'):
            period_names = get_period_names(context='format', width=width,
                                            locale=self.locale)
    ...
    def format_timezone(self, char, num):
        ...
        # TODO: To add support for O:1
            ...
            if num == 1:
                ...
                return get_timezone_name(self.value.tzinfo, locale=self.locale,
                                         return_zone=True)
            elif num == 3:
                return get_timezone_location(self.value.tzinfo, locale=self.locale,
                                             return_city=True)
            return get_timezone_location(self.value.tzinfo, locale=self.locale)
        # Included additional elif condition to add support for 'Xx' in timezone
        # format
        elif char == 'X':
            if num == 1:
                return get_timezone_gmt(self.value, width='iso8601_short',
                                        locale=self.locale,
                                        return_z=True)
            elif num in (2, 4):
                return get_timezone_gmt(self.value, width='short',
                                        locale=self.locale,
                                        return_z=True)
            elif num in (3, 5):
                return get_timezone_gmt(self.value, width='iso8601',
                                        locale=self.locale,
                                        return_z=True)
        elif char == 'x':
            if num == 1:
...
PATTERN_CHARS = {
    ...
    'E': [1, 2, 3, 4, 5, 6], 'e': [1, 2, 3, 4, 5, 6], 'c': [1, 3, 4, 5, 6],
    # week day
    'a': [1],  # period
...
def split_interval_pattern(pattern):
    ...
    > The pattern is then designed to be broken up into two pieces by determining
...
def match_skeleton(skeleton, options, allow_different_fields=False):
    ...
    # http://source.icu-project.org/repos/icu/icu4j/trunk/main/classes/core/src
    # /com/ibm/icu/text/DateIntervalInfo.java

    # Filter out falsy values and sort for stability; when `interval_formats` is
    # passed in, there may be a None key.
        ...
        for field in PATTERN_CHARS:
            ...
            if opt_width == 0 or input_width == 0:
                if not allow_different_fields:  # This one is not okay
                    option = None
                    break
                distance += 0x1000  # Magic weight constant for "entirely
                # different fields"
            elif field == 'M' and ((input_width > 2 and opt_width <= 2) or (
                    input_width <= 2 and opt_width > 2)):
                distance += 0x100  # Magic weight for "text turns into a number"
            else:
                distance += abs(input_width - opt_width)

        if not option:  # We lost the option along the way (probably due to
